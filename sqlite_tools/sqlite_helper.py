...
class Sqlite_Helper(object):
	...
	def set_db(self):
		...
		self.conn.create_function("HAVERSINE",4,self.haversine)
		self.conn.create_function("PCT",2,self.pct)
		self.conn.create_function("MEAN",-1,self.mean)
		self.conn.create_function("DIVERSITY_RACE",-1,self.diversity_race)
		self.conn.create_function("DIVERSITY_AGE",-1,self.diversity_age)
		self.conn.create_function("PERCENTILE",-1,self.percentile)
		self.conn.create_function("GREATERTHANPERCENTILE",3,self.greater_than_percentile)
		self.conn.create_function("EDU",1,self.edu)
		self.conn.create_function("POLITICALBOUNDARY",3,self.political_boundary)
		self.conn.row_factory = sdb.Row
